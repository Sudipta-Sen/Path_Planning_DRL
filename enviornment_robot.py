import numpy as np
from numpy import linalg as LA #low level efficient implementation of linear algebra algorithm
import cv2
# start with a resel
...
class CarDCENV(object):
	...
	def _update_sensor(self):
	  	...
	  	if not(np.isnan(cx) or np.isnan(cy)):
		  	...
		  	# Measuring distances with obstacles and to the window
		  	for i in range(1,self.sensor_max+1):
		  		...
		  		for sensor_n in range(len(kernel_s_cur)):
		  			...
		  			if 0<round(kernel_s_cur[sensor_n][1])<self.window[1] and 0<round(kernel_s_cur[sensor_n][0])<self.window[0]:
			  			if self.mapa[int(round(kernel_s_cur[sensor_n][1])),int(round(kernel_s_cur[sensor_n][0]))]==255: # the order of X,Y to Y,X is reversed, since map is an array
			  		...
			  		# Distance to the limits of the window
			  		else:
			...
			# Checking collision with window
		  	...
		  	# Checking if it reached the target
